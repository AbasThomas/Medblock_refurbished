...
export class IdentityController {
    ...
    async createPatient(@Body() createPatientDto: CreatePatientDto) {
        try {
            console.log('Creating patient with email:', createPatientDto.email);
            const bcrypt = require('bcrypt');

            const existingPatient = await this.patientRepository
                .createQueryBuilder('patient')
                .where(`patient.telecom::jsonb @> :telecom::jsonb`, {
                    telecom: JSON.stringify([{ system: 'email', value: createPatientDto.email }])
                })
                .getOne();
            ...
            return {
                ...
                accessToken: this.jwtService.sign({ did: didResult.did, role: 'patient' }),
            };
        } catch (error) {
    ...
    async loginPatient(@Body() body: { email: string; password: string }) {
        ...
        return {
            ...
            accessToken: this.jwtService.sign({ did: patient.did, role: 'patient' })
        };
    }
